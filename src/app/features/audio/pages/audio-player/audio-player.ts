...
import { playbackErrorMessage, toUserMessage } from '../../utils/api-error';
...
export class AudioPlayer implements OnInit {
  ...
  public play(): void {
    this.safePlay(this.audioPlayer.nativeElement);
  }
  ...
  public onAudioError(): void {
    this.errorMessage.set(playbackErrorMessage());
    this.isPlaying.set(false);
  }
      ...
      this.audioService.postAudioFile(draft.file, draft.name).subscribe({
        ...
        error: (err: unknown) => {
          ...
          if (!firstFailureMessage) {
            firstFailureMessage = toUserMessage('upload', err);
          }
  ...
  public deleteAudio(track: AudioDataModel): void {
    ...
    this.audioService.deleteSong(track.id).subscribe({
      ...
      error: (err: unknown) => {
        this.errorMessage.set(toUserMessage('delete', err));
      },
    });
  }
  ...
  public getAllAudioDatas(): void {
    ...
    this.audioService.getAllAudioDatas().subscribe({
      ...
      error: (err: unknown) => {
        this.loading.set(false);
        this.errorMessage.set(toUserMessage('list', err));
      },
    });
  }
  ...
  private finishUploadBatch(
    ...
    if (failed === 0) {
      ...
      this.uploadMessage.set('');
      this.errorMessage.set(firstFailureMessage ?? toUserMessage('upload', null));
    } else {
  ...
  private safePlay(player: HTMLAudioElement): void {
